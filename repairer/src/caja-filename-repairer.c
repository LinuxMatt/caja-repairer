...
static void
on_rename_menu_item_activated(CajaMenuItem *item, gpointer *data)
{
    gboolean res;
    gchar* new_name;
    GFile* file;
...
static void
on_repair_dialog_activated(CajaMenuItem* item, gpointer data)
{
    GList* files;
    guint i;
    guint n;
    ...
    argv[0] = g_strdup("caja-filename-repairer");

    i = 1;
    while (files != NULL) {
	GFile* file;

	file = caja_file_info_get_location(files->data);
...
repair_dialog_menu_item_new(GList* files)
{
    ...
    files = caja_file_info_list_copy(files);

    item = caja_menu_item_new(name, label, tooltip, NULL);
	...
	GtkWidget* window, gboolean is_submenu)
{
    ...
    item = caja_menu_item_new(id, label, tooltip, NULL);
	     ...
	     const char* name, GFile* file, GtkWidget* window)
{
	...
	if (have_item == NULL) {
	    ...
	    caja_menu_append_item(submenu, item);
	...
	item = caja_menu_item_new(menu_id,
			      _("Select a filename"),
...
need_repair_dialog(GList* files)
{
    ...
    g_print("need_repair_dialog\n");
    while (files != NULL) {
	if (caja_file_info_is_directory(files->data))
	    return TRUE;

	name = caja_file_info_get_name(files->data);
...
static GList *
caja_filename_repairer_get_file_items(CajaMenuProvider *provider,
					  GtkWidget            *window,
					  GList                *files)
{
    CajaMenuItem* item;
...
static void
caja_filename_repairer_menu_provider_iface_init(CajaMenuProviderIface *iface)
{
    iface->get_file_items = caja_filename_repairer_get_file_items;
}

static void 
caja_filename_repairer_instance_init(CajaFilenameRepairer *instance)
{
}

static void
caja_filename_repairer_class_init(CajaFilenameRepairerClass *klass)
{
}

GType
caja_filename_repairer_get_type(void) 
{
    return filename_repairer_type;
}

void
caja_filename_repairer_register_type(GTypeModule *module)
{
    static const GTypeInfo info = {
	sizeof(CajaFilenameRepairerClass),
	(GBaseInitFunc)NULL,
	(GBaseFinalizeFunc)NULL,
	(GClassInitFunc)caja_filename_repairer_class_init,
	NULL, 
	NULL,
	sizeof(CajaFilenameRepairer),
	0,
	(GInstanceInitFunc)caja_filename_repairer_instance_init,
    };

    static const GInterfaceInfo menu_provider_iface_info = {
	(GInterfaceInitFunc)caja_filename_repairer_menu_provider_iface_init,
	NULL,
	NULL
    };

    g_print("repairer : caja_filename_repairer_register_type\n");
    filename_repairer_type = g_type_module_register_type(module,
						 G_TYPE_OBJECT,
						 "CajaFilenameRepairer",
						 &info, 0);
